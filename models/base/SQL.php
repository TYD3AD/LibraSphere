...
class SQL implements IDatabase
{
    ...
    {
        try {
            $stmt = SQL::getPdo()->prepare("SELECT libelle_auteur FROM {$this->tableName} t INNER JOIN realiser r ON t.idressource=r.id_ressource INNER JOIN auteur a ON r.id_auteur=a.id_auteur WHERE {$this->primaryKey} = ?");
            $stmt->execute([$id]);
            return $stmt->fetchAll(PDO::FETCH_OBJ);
        } catch (\Exception $e) {
            return null;
